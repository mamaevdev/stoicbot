...
# Create a fixture for parser
@pytest.fixture
def parser():
    return Parser()


# Create a fixture for data
...
def test_is_end_of_quote(parser):
    # Test case: line ends with END_QUOTE_SYMBOL
    assert parser._is_end_of_quote(f"This is a quote: {END_QUOTE_SYMBOL}", "Next line")

    # Test case: next line starts with START_SOURCE_SYMBOL and next five characters are uppercase
    assert parser._is_end_of_quote(
    ...
    assert not parser._is_end_of_quote("There is no end quote symbol", "Next line")


# Tests for Parser._parse_letter_date_title
def test_parse_letter_date_title(parser):
    ...
    assert parser._parse_letter_date_title(lines) == expected_result
...
def test_parse_page_text(parser):
    ...
    assert parser._parse_page_text(text) == expected_result
...
# Test the number of entries in result JSON file is equal to 366
def test_total_entries(json_data):
    assert len(json_data) == 366


# Tests for dates
def test_dates(json_data):
    dates = list(json_data.keys())
    assert dates[0] == "January 1"
    assert dates[7] == "January 8"  # Missing closing quote symbol
    assert dates[20] == "January 21"  # Complex stoic structure
    ...
    assert dates[91] == "April 1"
    assert dates[120] == "April 30"
    assert dates[121] == "May 1"
    assert dates[151] == "May 31"
    assert dates[152] == "June 1"
    assert dates[181] == "June 30"
    assert dates[182] == "July 1"
    assert dates[210] == "July 29"  # Missing start source symbol
    assert dates[212] == "July 31"
    assert dates[213] == "August 1"
    assert dates[243] == "August 31"
    assert dates[244] == "September 1"
    assert dates[273] == "September 30"
    assert dates[274] == "October 1"
    assert dates[304] == "October 31"
    assert dates[305] == "November 1"
    assert dates[334] == "November 30"
    assert dates[335] == "December 1"
    assert dates[365] == "December 31"


# Tests for titles
def test_titles(json_data):
    assert json_data["January 1"]["title"] == "CONTROL AND CHOICE"
    assert json_data["January 8"]["title"] == "SEEING OUR ADDICTIONS"
    assert json_data["January 21"]["title"] == "A MORNING RITUAL"
    assert json_data["February 7"]["title"] == "FEAR IS A SELF-FULFILLING PROPHECY"
    assert json_data["February 9"]["title"] == "YOU DON\u2019T HAVE TO HAVE AN OPINION"
    assert json_data["July 29"]["title"] == "A CURE FOR THE SELF"
    assert json_data["December 31"]["title"] == "GET ACTIVE IN YOUR OWN RESCUE"


# Tests for quotes
def test_quotes(json_data):
    assert (
        json_data["January 1"]["quote"]
        == "\u201cThe chief task in life is simply this: to identify and separate matters so that I can say clearly to myself which are externals not under my control, and which have to do with the choices I actually control. Where then do I look for good and evil? Not to uncontrollable externals, but within myself to the choices that are my own . . .\u201d"
    )
    assert (
        json_data["January 8"]["quote"]
    ...
    assert (
        json_data["December 31"]["quote"]
        == "\u201cStop wandering about! You aren\u2019t likely to read your own notebooks, or ancient histories, or the anthologies you\u2019ve collected to enjoy in your old age. Get busy with life\u2019s purpose, toss aside empty hopes, get active in your own rescue\u2014if you care for yourself at all\u2014and do it while you can.\u201d"
    )


# Tests for quote sources
def test_quote_sources(json_data):
    assert (
        json_data["January 1"]["quote_source"]
        == "\u2014EPICTETUS, DISCOURSES, 2.5.4\u20135"
    )
    assert (
        json_data["January 8"]["quote_source"]
        == "\u2014SENECA, MORAL LETTERS, 74.12b\u201313"
    )
    assert (
        json_data["January 21"]["quote_source"]
        == "\u2014EPICTETUS, DISCOURSES, 4.6.34\u201335"
    )
    assert json_data["February 7"]["quote_source"] == "\u2014SENECA, OEDIPUS, 992"
    assert (
        json_data["February 9"]["quote_source"]
        == "\u2014MARCUS AURELIUS, MEDITATIONS, 6.52"
    )
    assert json_data["July 29"]["quote_source"] == "\u2014SENECA, MORAL LETTERS, 111.2"
    assert (
        json_data["December 31"]["quote_source"]
        == "\u2014MARCUS AURELIUS, MEDITATIONS, 3.14"
    )


# Tests for explanations
def test_explanations(json_data):
    assert json_data["January 1"]["explanation"] == "blah blah blah"


# Tests for explanations
def test_explanations(json_data):
    assert (
        json_data["January 1"]["explanation"]
    ...
    assert (
        json_data["January 8"]["explanation"]
    ...
    assert (
        json_data["January 21"]["explanation"]
    ...
    assert (
        json_data["February 7"]["explanation"]
    ...
    assert (
        json_data["February 9"]["explanation"]
    ...
    assert (
        json_data["December 31"]["explanation"]
